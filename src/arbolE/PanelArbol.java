...
public class PanelArbol extends JPanel {
    ...
    // NUEVO: separación horizontal fija entre hojas consecutivas.
    ...
    public PanelArbol(Nodo raiz, Color colorNodo, int radioNodo, Color colorLinea, float grosorLinea) {
        ...
        this.ANCHO_SLOT = Math.max(100, radioNodo * 6); // NUEVO: separación mínima entre hojas

        setBackground(Color.WHITE);
    }

    public PanelArbol(Nodo raiz, HashMap<String, String> hash, ArbolRenderConfig config) {
        this(raiz,
                config != null ? config.getColorNodo() : Color.LIGHT_GRAY,
                config != null ? Math.max(1, config.getAnchoBorde() / 2) : 15,
                config != null ? config.getColorLinea() : Color.DARK_GRAY,
                config != null ? config.getAnchoLinea() : 2f);
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        // PASO 2: Dibujar TODAS las líneas primero (quedan al fondo)
